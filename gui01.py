...
class Todo(tk.Tk):
    def __init__(self, tasks=None):
        ...
        self.text_frame = tk.Frame(self)                  # janela principal

        self.scrollbar = tk.Scrollbar(self.tasks_canvas, orient='vertical', command=self.tasks_canvas.yview)

        self.tasks_canvas.configure(yscrollcommand=self.scrollbar.set)

        self.title("Lista de Pendências App v.1.0")
        ...
        todo1 = tk.Label(self.tasks_frame, text="--- Adicione Tarefa ---", bg="lightgrey", fg="black", pady=10)
    ...
    def remove_task(self, event):
        task = event.widget
        if msg.askyesno("Apagar?", "Delete " + task.cget("text") + "?"):
